...
?>
		...
		<div class="slim-card-title" <?php if($grupo_opx->obrigatorio == 1) { ?> style="color:#CC3300" <?php } ?> <?php if($grupo_opx->obrigatorio == 2) { ?> style="color:#0066FF" <?php } ?>> <?php print $grupo_opx->nomegrupo;?> - Até <?php if($grupo_opx->quantidade > 1) { print $grupo_opx->quantidade; ?> opções <?php } else { print $grupo_opx->quantidade; ?> opção <?php } ?> <?php if($grupo_opx->obrigatorio == 1) { ?> <span style="color:#FF0000">*</span><?php } ?>
		</div>
